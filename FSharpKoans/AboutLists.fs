...
module ``about lists`` =
    ...
    (* THINK ABOUT IT: In general, what performs better for building lists, 
       ...
       Concatenating with @ requires O(n) operations to traverse the linked
       list.  Concatenating with :: requires O(1) operations to create a 
       new node and a pointer to the first node in the list.*)
